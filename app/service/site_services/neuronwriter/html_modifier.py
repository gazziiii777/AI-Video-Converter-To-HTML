...
class HTMLModifier:
    ...
    async def replace_in_file(html: str, replacements: List[Dict]) -> str:
        ...
        try:
            ...
            logger.error(f"Ошибка в фунции replace_in_file: {e}")
            logger.debug(
                f"Аргументы переданный в фунцию replace_in_file, первый аргумент html: {html}\n Второй аргумент replacements: {replacements}")
            # return None
            raise
    ...
    async def process_html_with_h3(html: str) -> None:
        ...
        try:
            ...
            logger.error(f"Ошибка в фунции process_html_with_h3: {e}")
            logger.debug(
                f"Аргументы переданный в фунцию process_html_with_h3, первый аргумент html: {html}")
            # return None
            raise