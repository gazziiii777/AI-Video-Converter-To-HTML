...
from app.client.neurowriter import NeuroWriter  # Импортируй правильно свой модуль

async def main():
    # Пример чтения HTML-файла
    with open("combined.html", "r", encoding="utf-8") as file:
        html_content = file.read()

    # Создаём экземпляр и вызываем метод
    writer = NeuroWriter()
    result = await writer.import_content(html_content)
    print("Результат импорта:", result)
